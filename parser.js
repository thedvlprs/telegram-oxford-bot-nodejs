function mainEntriesToHtml(entries) {
  return entries.map(entry => {
    ...
    if (entry.subDefinitions) {
      subDefinitionsHtml = entry.subDefinitions
        .map(subDefinition => {
          return `
        » ${subDefinition}`;
        })
...
function parser(json) {
  ...
  const parseEntries = json.results[0].lexicalEntries.flatMap(lexicalEntry => {
    ...
    constructObject.entries = lexicalEntry.entries
      .map(entry => {
        return entry.senses
          .map(sense => {
            definitionCount++;
            ...
            if (sense.subsenses && Array.isArray(sense.subsenses)) {
              definitions.subDefinitions = sense.subsenses
                .map(subsense => subsense.definitions)
                .flat();
            }
            return definitions;
          })
